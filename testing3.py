...
def find_max(rects):
    ...
    for rect in rects:
        ...
        if min(w, h) > 0:
            ratio =w / h
            area = w * h
            if 0.8< ratio < 2.5 and area>10000  :  # 宽高比必须在1.5到2之间
                if area > max_size:
                    max_rect = rect
                    max_size = area
    return max_rect
...
#    binary_img.gaussian(3)
    ...
    if b+d+150<480:
        ...
        d= 480-b
    rects = binary_img.find_rects((a,b,c,d),threshold=5000)
...
#                gray_img = img1.to_grayscale()

#                gray_img.gamma_corr(1.3)
#                # 二值化处理
#                binary_img = gray_img.binary([(150, 255)])
                # for rgb888planar
